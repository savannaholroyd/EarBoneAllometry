...
ChamTree<-read.nexus("data/Tolley2013trimmedL.trees")
...
m0 <- slouch.fit(phy = ChamTree,
                 ...
                 mv.response = rep(0.1 * var(DataCham$logpter), length(ChamTree$tip.label)), #plus 5.5% intraspecific variation
                 mv.direct.cov = rep(0.1 * var(DataCham$logbsl), length(ChamTree$tip.label))) #plus 5.5% intraspecific variation
plot(m0)
summary(m0)
...
for (hyp in hypotheses){
  ...
  ape::write.nexus.data(l2, file = paste0("data/chameleons/regimes_", hyp, ".nex"), format = "standard")
}
